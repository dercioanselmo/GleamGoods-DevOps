...
## Known gap, discovered by diffing this module against the application repo's actual live workflows

The application repo's CI (`build-push-*.yaml` × 5, plus `codeql.yaml`, plus `dast-zap.yaml` — all 7 workflows) upload Trivy/Snyk/CodeQL/ZAP scan reports to S3 (`aws s3 cp ... s3://${{ vars.REPORT_BUCKET }}/...`), authenticated via **this module's** `github_actions` role. That role only has `AmazonEC2ContainerRegistryPowerUser` attached — **no S3 permission of any kind** — and this Terraform project provisions **no S3 bucket** for this purpose anywhere. Every one of those upload steps is wrapped in `|| echo "... upload skipped"`, so this fails silently rather than breaking CI, which is presumably why it's gone unnoticed. If implementing this module fresh and the report-upload behavior in those workflows is meant to actually work, this module needs: an S3 bucket for scan reports, and a scoped `s3:PutObject` policy (bucket-ARN-scoped, not `*`) attached to the *existing* `github_actions` role — not a new role. Don't add this speculatively without confirming first whether `REPORT_BUCKET` is actually set and whether reports are expected to land anywhere; it's possible this was a deliberately deferred/accepted gap, not an oversight to silently fix.

## Repo-wide conventions this module must follow

- File naming: `c1-versions.tf` (hyphen convention, matching `03`), `c2-variables.tf`, then split by concern like `03`'s addon-per-file pattern: `c3-01-ecr-repositories.tf` (just the ECR repos) and `c3-02-github-actions-iam.tf` (OIDC provider + role + policy attachment + the account-ID data source) — not combined into one file. `c4-outputs.tf` stays a single shared outputs file (same pattern as `01_remote_backend_s3bucket`/`02_VPC`), not split per resource.
