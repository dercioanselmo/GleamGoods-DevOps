...
# `store-stock` in the `konecta` namespace) to assume the shared IAM
# role that has access to the Konecta DB secret in AWS Secrets Manager,
# so the Secrets Store CSI Driver can mount it into the Store Stock Pod
# at runtime.
